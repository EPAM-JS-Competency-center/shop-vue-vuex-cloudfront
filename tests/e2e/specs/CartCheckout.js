...
const successCheckout = {
	items: [{ productId: '7567ec4b-b10c-48c5-9345-fc73c48a80a0', count: 1 }],
	address: {
		firstName: 'Vasiliy',
		lastName: 'Pupkin',
		address: 'Nevskiy st.',
...
describe('view: CartCheckout', () => {
	...
	it('Header: should contain cart link', () => {
		cy.get('header a[data-test-id="cart-link"]').should(
			'have.attr',
			'href',
			'/cart'
		);
	...
	it('Header: should contain account menu (3 items)', () => {
		cy.get('header [data-test-id="account-button"]').click();

		cy.get('[data-test-id="account-menu"] > a').should('have.length', 3);
	});
	...
	it('Page: should contain correct h4 title', () => {
		cy.get('main h4').should('contain', 'Checkout');
	});

	it('Scenario: pass every step one-by-one', () => {
		cy.get('[data-test-id="shop-button"]').click();

		cy.intercept('PUT', '/dev/profile/cart/', []);

		cy.intercept('/dev/product/available/', products).as(
		...
		cy.get('[data-test-id="to-2-step-button"]').click();

		// fill shipping address form
		cy.get(`input[name="firstName"]`).type('Vasiliy');

		cy.get(`input[name="lastName"]`).type('Pupkin');

		cy.get(`input[name="address"]`).type('Nevskiy st.');

		cy.get(`textarea[name="comment"]`).type('my first purchase');

		cy.get('[data-test-id="to-3-step-button"]').click();
